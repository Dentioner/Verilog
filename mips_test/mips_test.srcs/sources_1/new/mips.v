...
module mips_cpu(
	...
	localparam sw_out     = 9'bx1x001000;
	localparam beq_out    = 9'bx0x000101;
	localparam addiu_out  = 9'b010100000;
	localparam bne_out    = 9'bx0x000101;
...
begin
	...
	alu alu1(.A(alu1_a), .B(alu1_b), .ALUop(ALUop), .Zero(Zero_raw), .result(alu1_result));//overflow 和 carryout的信号暂时没引出
	alu alu2(.A(alu2_a), .B(alu2_b), .ALUop(ADD), .result(alu2_result));//Zero, overflow 和 carryout的信号暂时没引出, 此alu一直当做加法器使用
